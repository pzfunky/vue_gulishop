        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- 
                  blur事件不会去判断这次输入的内容和之前的数据是否一样再触发
                  input的change事件内部包含了失去焦点事件,当用户失去焦点的时候,也会选择性触发change事件
              ...
              <div class="add">
                <!-- 
                  加入购物车不能直接跳转,而是需要先发请求给后台,后台需要把这个购物车信息存储数据库,
                  请求成功后,会返回给我们信息,我们根据这个信息再去跳转,否则有可能添加数据库失败,
                  而我们已经跳转到下个页面,就会出问题
                 -->
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      async addShopCart(){
        //第一步:先发请求添加购物车
        //第一种写法 actions的异步函数,返回的promise永远是成功的,要看actions异步函数的return
